...
def load_model_from_config(config, ckpt, delta_ckpt=None, verbose=True, freeze=True):
    ...
    if delta_ckpt is not None:
        token_weights1 = sd['conditioner.embedders.0.transformer.text_model.embeddings.token_embedding.weight']
        token_weights2 = sd['conditioner.embedders.1.model.token_embedding.weight']
    ...
    if delta_ckpt is not None:
        ...
        model.conditioner.embedders[0].transformer.text_model.embeddings.token_embedding.weight.data = torch.cat([token_weights1, sd_delta['embed'][0]], 0).to(model.device)
        model.conditioner.embedders[1].model.token_embedding.weight.data = torch.cat([token_weights2, sd_delta['embed'][1]], 0).to(model.device)
        del sd_delta['embed']
